...
public class MyEvaActivity extends BaseActivity {
    ...
    private void initRec() {
        ...
        mAdapter = new ShopEvaAdapter(R.layout.item_shop_eva, shopEvaBeanList);
            ...
            public void onItemImgClick(BaseQuickAdapter adapter, View view, int position, int f_pos) {
                ...
                switch (mediaType) {
                    case 1:
                        ...
                        PictureSelector.create(MyEvaActivity.this).themeStyle(R.style.picture_default_style).openExternalPreview(position, shopEvaBeanList.get(f_pos).getLocalMedia());
                        break;
                }
            }
        });
    ...
    protected void initData() {
        SubscriberOnNextListener<BaseBean<ShopEvaBean>> onNextListener = baseBean -> {
            if (HttpMethods.SUCCESS_CODE == baseBean.getCode()) {
                if (baseBean.getData().getPinglun().isEmpty()) {
                    if(page==1){
                    ...
                    shopEvaBeanList.addAll(baseBean.getData().getPinglun());
                    for (int j = 0; j < shopEvaBeanList.size(); j++) {
                        ...
                        for (int i = 0; i < shopEvaBeanList.get(j).getImages().size(); i++) {
                            ...
                            localMedia.setPath(baseBean.getData().getPinglun().get(j).getImages().get(i));
        ...
        map.put("page", page);
        HttpMethods.getInstance().pinglun(new ProgressSubscriber<>(onNextListener, this, isShowDialog), map);
    }
